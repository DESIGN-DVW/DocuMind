...
### docs/diagrams/ (per-repo):

- Purpose: Generated Mermaid diagram files (.mmd) and rendered PNGs

- Generated: Yes (created by mermaid-processor.mjs, output to each repo's `docs/diagrams/`)

- Committed: Yes (diagram sources and rendered outputs are committed per-repo)
